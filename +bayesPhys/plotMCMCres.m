...
function plotMCMCres_single(S,x1,tc_func_name,col)
    ...
    for i=1:length(S.P1)
        y2(i,:)=bayesPhys.getTCval(x1,tc_func_name,[S.P1(i) S.P2(i) S.P3(i) S.P4(i)])';
%         plot(x1,y2,'Color',col,'FaceAlpha',(S.log_llhd(i)-min(S.log_llhd))./(max(S.log_llhd)-min(S.log_llhd)));
    end
    plot(x1,y2,'Color',col);
    hold on
    y1=bayesPhys.getTCval(x1,tc_func_name,[S.P1_median S.P2_median S.P3_median S.P4_median]);

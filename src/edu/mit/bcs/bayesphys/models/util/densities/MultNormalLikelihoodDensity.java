...
public class MultNormalLikelihoodDensity extends SingleResponseLikelihoodDensity {
	NormalArchetype dist = Distributions.defaultFactory().normalArchetype();
	public final Link<Density, DoubleState, String> multiplier = declareDependency("multiplier");
	
	@Override
	public double computeSingleLogDensity(double modelVal, double dataVal) {
		return dist.logPdf(dataVal, modelVal, modelVal * multiplier.get().getValue());
	}
}
